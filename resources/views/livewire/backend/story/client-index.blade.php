<div class="mt-8 flex flex-col">
    <x-auth-session-status class="mb-4" :status="session('client_index')" />
        ...
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-6 gap-6 mb-5">
                ...
                <div class="pagination flex justify-end col-span-3 sm:col-span-3 gap-1">
                    @if ($total > $perPage)
                        @if ($currentPage > 3)
                        <button wire:click="paginate(0)" class="{{ $start / $perPage == 0 ? 'bg-gray-600' : 'bg-indigo-600' }} inline-flex items-center justify-center rounded-md border border-transparent px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">
                            1
                        </button>
                        @endif
                        ...
                        <button wire:click="paginate({{ ceil($total / $perPage) - 1 }})" class="{{ $start / $perPage == ceil($total / $perPage) - 1 ? 'bg-gray-600' : 'bg-indigo-600' }} inline-flex items-center justify-center rounded-md border border-transparent px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">
                            {{ ceil($total / $perPage) }}
                        </button>
                        @endif
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    @if ($client->image == '' || $client->image == null)
                                        <img src="{{ asset('upload/no_image.jpg') }}" style="height: 50px; width:70px;"
                                            alt="No Image">
                                    @else
                                        <img src="{{ URL::to('storage/' . $client->image) }}"
                                            style="height: 50px; width:70px;">
                                    @endif
                                </td>
                                <td  class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $client->priority }}</td>
                                <td
                                    class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                    <div class="flex items-center justify-center">
                                      
                                            @if ($client->status == 1)
                                                <button wire:click="activate({{ $client->id }})"
            ...
            @if ($client instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator)
                <div class="mb-5">
                    {{ $client->links() }}
                </div>
            @endif
